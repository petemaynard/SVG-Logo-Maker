...
const questions = include('questions');
...
class Square extends Shape {
   ...
   buildSVG2line = `<rect width="300" height="200" fill="${bgColor}"/>`;
}
...
class Circle extends Shape {
   ...
   buildSVG2line = `<circle cx="150" cy="100" r="80" fill="${bgColor}"/>`;
}
...
class Triangle extends Shape {
   ...
   buildSVG2line = `<polygon points="150,0 0,200 300,200" fill="${bgColor}"/>`;
}
...
inquirer.prompt(questions)
   .then(answers => {
      ...
      // Should the above reference questions.*
